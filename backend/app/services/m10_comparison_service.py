...
def _check_run_integrity(
    ...
    if run.status != "success_complete" or [row.month for row in rows] != expected_months:
        _block("comparison_fingerprint_invalid", "persisted monthly result set is invalid")
    ...
    for domain in domains:
        ...
        for candidate in domain["candidates"]:
            ...
            if domain_id == "m06_monthly_pension":
                ...
                values = {
                    component.get("provenance", {}).get("handoff_contract_version")
                    for component in components
                    if isinstance(component, dict) and isinstance(component.get("provenance"), dict)
                }
                if values != {M06_HANDOFF_CONTRACT}:
...
def _money(value: Any, *, delta: bool = False) -> tuple[Decimal, str]:
    ...
    text = format(value, ".2f") if isinstance(value, Decimal) else value
...
def compare_runs(
    ...
    if reference_months != expected or compared_months != expected or reference_months != compared_months:
