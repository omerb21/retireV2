...
def test_success_is_exact_stateless_and_deterministic(api):
    ...
    assert body["comparison_contract_version"] == "m10-scenario-comparison-v2"
    assert body["pair_admission_contract"] == "m10-pair-admission-v2"
    assert body["comparison_result_schema"] == "m10-comparison-result-v2"
    assert body["comparison_fingerprint_schema"] == "m10-comparison-fingerprint-v2"
    assert set(body) == {
...
def test_missing_month_membership_is_owned_by_predecessor_currentness(api, side):
    client, _, engine = api
    _, _, reference, compared = _pair(client)
    ...
    assert response.json()["detail"] == {
        "code": "comparison_run_not_current",
        "message": f"{side} run is not current",
    }


def test_extra_and_incomplete_membership_is_owned_by_compared_currentness(api):
    client, _, engine = api
    _, _, reference, compared = _pair(client)
    with engine.begin() as connection:
        row = connection.exec_driver_sql(
            "SELECT component_evidence, gross_inflow_total, gross_outflow_total, period_net "
            "FROM m09_subject_monthly_results WHERE run_id=? AND month=?",
            (compared["run_id"], "2026-03"),
        ).mappings().one()
        component_evidence = row["component_evidence"]
        if isinstance(component_evidence, str):
            component_evidence = json.loads(component_evidence)
        payload = {
            "month": "2026-04",
            "component_evidence": component_evidence,
            "gross_inflow_total": f'{Decimal(str(row["gross_inflow_total"])):.2f}',
            "gross_outflow_total": f'{Decimal(str(row["gross_outflow_total"])):.2f}',
            "period_net": f'{Decimal(str(row["period_net"])):.2f}',
        }
        connection.exec_driver_sql(
            "UPDATE m09_subject_monthly_results SET month=?, result_fingerprint=? "
            "WHERE run_id=? AND month=?",
            (
                "2026-04",
                factual_service._digest(payload),
                compared["run_id"],
                "2026-03",
            ),
        )
    ...
    assert response.json()["detail"] == {
        "code": "comparison_run_not_current",
        "message": "compared run is not current",
    }
...
def test_valid_current_pair_passes_defensive_month_alignment_guard(api):
    client, _, _ = api
    _, _, reference, compared = _pair(client)
    response = client.post("/api/clients/1/m10/compare", json=_request(reference, compared))
    assert response.status_code == 200, response.text
    assert [row["month"] for row in response.json()["monthly_comparisons"]] == [
        "2026-01",
        "2026-02",
        "2026-03",
    ]
...
@pytest.mark.parametrize(
    ("reference_months", "compared_months"),
    [
        (["2026-01", "2026-02", "2026-03"], ["2026-01", "2026-02"]),
        (["2026-01", "2026-02", "2026-03"], ["2026-01", "2026-02", "2026-04"]),
        (["2026-01", "2026-02", "2026-03"], ["2026-02", "2026-01", "2026-03"]),
    ],
)
def test_defensive_month_alignment_helper_fails_closed_for_unequal_canonical_arrays(
    reference_months, compared_months
):
    # Structural guard evidence only: this does not represent a valid persisted
    # PKG-014 pair and does not bypass predecessor currentness or eligibility.
    with pytest.raises(M09CashflowError) as captured:
        comparison_service._assert_month_alignment(
            reference_months,
            compared_months,
            ["2026-01", "2026-02", "2026-03"],
        )
    assert captured.value.code == "comparison_month_alignment_mismatch"


def test_numeric_domain_remains_after_valid_defensive_month_alignment_guard():
    months = ["2026-01", "2026-02", "2026-03"]
    comparison_service._assert_month_alignment(months, list(months), list(months))
    with pytest.raises(M09CashflowError) as captured:
        comparison_service._metric(Decimal("1.001"), Decimal("0.00"))
...
@pytest.mark.parametrize(
    ...
    [
        ...
        [
            {
                "domain_identity": "m06_monthly_pension",
                "candidates": [_candidate("a", components=[{"provenance": []}])],
            }
        ],
        [
            {
