...
const detailedSubject = (current = revision()): M06Subject => ({ ...subject(1), current_revision: current, eligibility: { subject_id: "subject-1", eligible_for_downstream: current.state === "resolved" || current.state === "warning_reviewed", current_revision_id: current.revision_id, exclusion_reasons: current.state === "blocked" ? ["conversion_blocked"] : [], informational_warnings: current.informational_warnings, meaning: "technically eligible under the bounded PKG-011 M06 contract" } });
type Deferred<T> = { promise: Promise<T>; resolve(value: T): void };
const deferred = <T,>(): Deferred<T> => { let resolve!: (value: T) => void; const promise = new Promise<T>((yes) => { resolve = yes; }); return { promise, resolve }; };
