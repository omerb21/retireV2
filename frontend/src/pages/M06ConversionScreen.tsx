...
export function M06ConversionScreen() {
  ...
  return <main><h2>M06 First-stage pension/capital conversion</h2>
    ...
    {error ? <p role="alert">{error}</p> : null}{busyCount > 0 ? <p>Loading M06 evidence…</p> : null}
    <section><h3>Eligible predecessor inputs</h3>{candidates.length ? <ul>{candidates.map((row) => <li key={row.candidate_id}><button type="button" onClick={() => setSelected(row)}>{row.provider_name} / {row.account_reference} / {row.mode}</button> — {row.formula_id}; amount {row.input_amount ?? "missing"}; date {row.input_date ?? "missing"}; eligible {String(row.eligible)}</li>)}</ul> : <p>No M06 candidates.</p>}
      {selected ? <div><h4>Coefficient evidence and applicability</h4><p>Formula: {selected.formula_id}. Input and result are server-derived and cannot be edited.</p>
        ...
        <button disabled={busyCount > 0 || !coefficient || !reason} onClick={() => void mutate(() => startM06(clientId, selected, intent()))}>Start immutable draft</button></div> : null}</section>
    ...
    {current ? <section><h3>Current conversion revision</h3><p>Revision {current.revision_sequence}: {current.state}; {current.formula_id}; input {current.input_amount ?? "missing"}; coefficient {current.coefficient.coefficient} ({current.coefficient.authority_class}).</p><p>Actor {current.actor} is operational provenance, not authentication or approval.</p><p>Warnings: {warningIds.join(", ") || "none"}; blockers: {current.blocking_reasons.join(", ") || "none"}.</p>
      ...
      <button disabled={busyCount > 0 || current.state !== "draft" || current.action_type === "resolve"} onClick={() => void mutate(() => resolveM06(clientId, current.subject_id, current.revision_id))}>Resolve exact formula</button>
      <button disabled={busyCount > 0 || current.state !== "draft" || current.action_type !== "resolve" || warningIds.length === 0 || !actionReason} onClick={() => void mutate(() => reviewM06Warnings(clientId, current.subject_id, current.revision_id, warningIds, actionReason))}>Review exact mandatory warning set</button>
      <button disabled={busyCount > 0 || !coefficient || !reason || !actionReason || current.state === "superseded"} onClick={() => void mutate(() => correctM06Coefficient(clientId, current.subject_id, current.revision_id, intent(), actionReason))}>Append corrected coefficient draft</button>
      <button disabled={busyCount > 0 || !actionReason || current.state === "superseded"} onClick={() => void mutate(() => supersedeM06(clientId, current.subject_id, current.revision_id, actionReason))}>Supersede current conversion</button>
    </section> : null}
