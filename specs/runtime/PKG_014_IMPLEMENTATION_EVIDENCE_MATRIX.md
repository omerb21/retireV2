...
| Actual channel | Stale success | Plain rejection | Structured API error | Stale `finally` / loading ownership |
|---|---|---|---|---|
| `subject-list` | `ignores stale candidate success after client switch` | `suppresses stale subject-list plain rejection and preserves the newer client loading owner` | `suppresses stale subject-list structured rejection and preserves the newer client loading owner` | Both rejection variants and the success test assert the newer client spinner/list owner remains effective. |
| `baseline-resolution` | `suppresses stale baseline success after A-to-B and preserves the newer client loading owner` | `suppresses stale baseline-resolution plain rejection and preserves the newer client loading owner` | `suppresses stale baseline-resolution structured rejection and preserves the newer client loading owner` | Each test settles A-old while B loading remains visible and owned. |
| `subject-creation` | `keeps stale create response from selecting a subject after client change` | `suppresses stale subject-creation plain rejection and preserves the newer client loading owner` | `suppresses stale subject-creation structured rejection and preserves the newer client loading owner` | Rejection variants directly retain the newer spinner and prohibit stale selection/error. |
| `subject-detail` (detail plus initial history) | `invalidates subject A detail success and finally immediately on A-to-B` | `suppresses stale subject-detail plain rejection and preserves the newer subject loading owner` | `suppresses stale subject-detail structured rejection and preserves the newer subject loading owner` | The B detail remains pending and its spinner survives every A-old settlement. |
| `subject-execution` | `does not let stale subject execution write result or clear new-subject loading` | `suppresses stale subject-execution plain rejection and preserves the newer subject loading owner` | `suppresses stale subject-execution structured rejection and preserves the newer subject loading owner` | Stale execution cannot write a run or clear the B detail loading owner. |
| `run-history` (independent post-execution refresh) | `keeps independently refreshed stale run history out of the next subject and preserves its loading` | `suppresses independently refreshed stale run-history plain rejection and preserves newer loading` | `suppresses independently refreshed stale run-history structured rejection and preserves newer loading` | Stale history cannot enter B, surface an error, or clear B loading. |
| `run-result` (composite result/currentness/eligibility) | `keeps a stale run-result success out of the next subject and preserves its loading` | `suppresses stale run-result plain rejection and preserves the newer subject loading owner` | `suppresses stale run-result structured rejection and preserves the newer subject loading owner` | Stale composite result/currentness/eligibility cannot render or release B loading. |

### Direct A→B→A matrix

| Actual channel | Stale success | Plain rejection while A-new pending | Structured API error while A-new pending | Direct stale `finally` / loading proof |
|---|---|---|---|---|
| `subject-list` | `keeps an old subject-list A result out of a new A client generation` | `keeps A-new subject-list pending through A-old plain rejection after A-to-B-to-A` | `keeps A-new subject-list pending through A-old structured API error after A-to-B-to-A` | Both rejection tests assert the spinner and disabled baseline action before and after A-old settles, then resolve A-new and assert only its list. |
| `baseline-resolution` | `keeps an old baseline success out of a new A client generation` | `keeps A-new baseline-resolution pending through A-old plain rejection after A-to-B-to-A` | `keeps A-new baseline-resolution pending through A-old structured API error after A-to-B-to-A` | Both rejection tests assert the spinner and disabled baseline action survive A-old `finally`; only then is A-new resolved and selected. |
| `subject-creation` | `keeps an old create success from appending or selecting into a new A client generation` | `keeps A-new subject-creation pending through A-old plain rejection after A-to-B-to-A` | `keeps A-new subject-creation pending through A-old structured API error after A-to-B-to-A` | Both rejection tests directly retain the spinner and disabled create action; stale subject/error remains absent before A-new resolves. |
| `subject-detail` (detail plus initial history) | `keeps an old subject-detail success out of a new A subject generation` | `keeps A-new subject-detail pending through A-old plain rejection after A-to-B-to-A` | `keeps A-new subject-detail pending through A-old structured API error after A-to-B-to-A` | Both rejection tests keep A-new detail unresolved, assert spinner/disabled baseline action after A-old `finally`, and then adopt only A-new detail. |
| `subject-execution` | `keeps an old execution success from overwriting a new A result after A-to-B-to-A` | `keeps A-new subject-execution pending through A-old plain rejection after A-to-B-to-A` | `keeps A-new subject-execution pending through A-old structured API error after A-to-B-to-A` | Both rejection tests assert no result/error and retain the spinner plus disabled execute action until A-new produces its uniquely marked run. |
| `run-history` (independent post-execution refresh) | `keeps independently refreshed old A history out of a new A subject generation` | `keeps A-new run-history pending through A-old plain rejection after A-to-B-to-A` | `keeps A-new run-history pending through A-old structured API error after A-to-B-to-A` | Both rejection tests create real old/new post-execution history refreshes, retain spinner/disabled execute action, and admit only A-new history after old `finally`. |
| `run-result` (composite result/currentness/eligibility) | `keeps an old run-result composite out of a new A subject generation` | `keeps a real A-new run-result composite pending through A-old plain rejection after A-to-B-to-A` | `keeps a real A-new run-result composite pending through A-old structured API error after A-to-B-to-A` | Both rejection tests prove two real composite calls, retain spinner/disabled execute action with no result/currentness/eligibility/error after A-old `finally`, and only then resolve the uniquely marked A-new composite. |

The A→B→A rejection rows use this mandatory order: start A-old, visit B, return to A, start a real A-new request, keep A-new pending, reject A-old, assert error absence and continued A-new loading/button ownership, then resolve A-new and assert its uniquely identifiable state. Thus plain rejection, structured rejection, and the associated stale `finally` are exercised directly rather than inferred from a later final state.
