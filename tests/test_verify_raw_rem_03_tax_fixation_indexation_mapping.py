...
SCRIPT = ROOT / "scripts/verify_raw_rem_03_tax_fixation_indexation_mapping.py"
FILES = (
    ...
    Path("specs/runtime/raw_remediation/RAW_REM_03_HIGH_RISK_TAX_FIXATION_INDEXATION_MAPPING.md"),
...
def test_current_raw_rem_03_passes() -> None:
    result = run(ROOT)
    assert result.returncode == 0, result.stdout + result.stderr


def test_scope_decision_removed_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[4], lambda text: re.sub(r"^\| V1LOGIC-005 \|.*\n", "", text, count=1, flags=re.MULTILINE))
...
def test_raw_rem_01_decision_added_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[4], lambda text: text.replace("RAW_REM_03_HIGH_RISK_TAX_FIXATION_INDEXATION_DECISIONS_CREATED", "| V1LOGIC-001 | Annual tax parameters | TAXMAP_NEEDS_MANUAL_SOURCE_REVIEW | YES | Manual review | RAW-REM-03-PATCH | V1 archive source is uncertain. |\n\nRAW_REM_03_HIGH_RISK_TAX_FIXATION_INDEXATION_DECISIONS_CREATED"))
    result = run(repo_copy)
    assert result.returncode != 0
    assert "RAW_REM_01_ID_IN_DECISIONS" in result.stdout


def test_raw_rem_02_only_decision_added_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[4], lambda text: text.replace("RAW_REM_03_HIGH_RISK_TAX_FIXATION_INDEXATION_DECISIONS_CREATED", "| V1LOGIC-006 | Tax validation / warnings / missing info | TAXMAP_NEEDS_BEHAVIOR_CONTRACT | YES | Behavior contract | RAW-REM-03-PATCH | V1 archive row belongs only to RAW-REM-02. |\n\nRAW_REM_03_HIGH_RISK_TAX_FIXATION_INDEXATION_DECISIONS_CREATED"))
    result = run(repo_copy)
    assert result.returncode != 0
    assert "RAW_REM_02_ONLY_ID_IN_DECISIONS" in result.stdout
...
def test_needs_outcome_must_block_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[4], lambda text: replace_row(text, 7, lambda cells: cells[2].startswith("TAXMAP_NEEDS_"), lambda cells: cells.__setitem__(3, "NO")))
    result = run(repo_copy)
    assert result.returncode != 0
...
def test_unclassified_fails_pass_package(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[4], lambda text: replace_row(text, 7, lambda cells: True, lambda cells: cells.__setitem__(2, "TAXMAP_UNCLASSIFIED_FAIL")))
    result = run(repo_copy)
    assert result.returncode != 0
    assert "DECISION_UNCLASSIFIED_IN_PASS_PACKAGE" in result.stdout


def test_formula_inventory_row_removed_fails(repo_copy: Path) -> None:
    mapping = FILES[3]
    mutate(repo_copy, mapping, lambda text: remove_row(text, 9, lambda cells: cells[0] == "V1LOGIC-009"))
    result = run(repo_copy)
    assert result.returncode != 0
    assert "FORMULA_INVENTORY_MISSING" in result.stdout


def test_numeric_expected_output_without_source_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[3], lambda text: replace_row(text, 7, lambda cells: cells[1].startswith("RAW03-GOLDEN-"), lambda cells: cells.__setitem__(3, "123")))
    result = run(repo_copy)
    assert result.returncode != 0
    assert "GOLDEN_EXPECTED_OUTPUT_SOURCE_INVALID" in result.stdout


@pytest.mark.parametrize("phrase", ["assumed", "probably", "standard tax rule", "by law generally"])
def test_unsupported_formula_language_fails(repo_copy: Path, phrase: str) -> None:
    ...
    result = run(repo_copy)
    assert result.returncode != 0
    assert "UNSUPPORTED_FORMULA_LANGUAGE" in result.stdout
...
def test_fail_marker_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[3], lambda text: text.replace("RAW_REM_03_HIGH_RISK_TAX_FIXATION_INDEXATION_MAPPING_PASS", "RAW_REM_03_HIGH_RISK_TAX_FIXATION_INDEXATION_MAPPING_FAIL"))
    result = run(repo_copy)
    assert result.returncode != 0
