...
SCRIPT = ROOT / "scripts/verify_raw_rem_04_clearinghouse_parser_balance_ledger_mapping.py"
FILES = (
    ...
    Path("specs/runtime/raw_remediation/RAW_REM_04_CLEARINGHOUSE_PARSER_BALANCE_LEDGER_MAPPING.md"),
...
def test_current_raw_rem_04_passes() -> None:
    result = run(ROOT)
    assert result.returncode == 0, result.stdout + result.stderr


def test_scope_decision_removed_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[4], lambda text: re.sub(r"^\| V1LOGIC-053 \|.*\n", "", text, count=1, flags=re.MULTILINE))
...
def test_raw_rem_01_decision_added_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[4], lambda text: text.replace("RAW_REM_04_CLEARINGHOUSE_PARSER_BALANCE_LEDGER_DECISIONS_CREATED", "| V1LOGIC-001 | Clearinghouse intake | CLRMAPP_NEEDS_MANUAL_SOURCE_REVIEW | YES | Manual review | RAW-REM-04-PATCH | V1 archive source is uncertain. |\n\nRAW_REM_04_CLEARINGHOUSE_PARSER_BALANCE_LEDGER_DECISIONS_CREATED"))
    result = run(repo_copy)
    assert result.returncode != 0
    assert "RAW_REM_01_ID_IN_DECISIONS" in result.stdout


@pytest.mark.parametrize("logic_id", ["V1LOGIC-006", "V1LOGIC-005"])
def test_prior_package_only_decision_added_fails(repo_copy: Path, logic_id: str) -> None:
    mutate(repo_copy, FILES[4], lambda text: text.replace("RAW_REM_04_CLEARINGHOUSE_PARSER_BALANCE_LEDGER_DECISIONS_CREATED", f"| {logic_id} | Clearinghouse intake | CLRMAPP_NEEDS_BEHAVIOR_CONTRACT | YES | Behavior contract | RAW-REM-04-PATCH | V1 archive row belongs to a prior package only. |\n\nRAW_REM_04_CLEARINGHOUSE_PARSER_BALANCE_LEDGER_DECISIONS_CREATED"))
    result = run(repo_copy)
    assert result.returncode != 0
    assert "RAW_REM_02_OR_03_ONLY_ID_IN_DECISIONS" in result.stdout
...
def test_needs_outcome_must_block_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[4], lambda text: replace_row(text, 7, lambda cells: cells[2].startswith("CLRMAPP_NEEDS_"), lambda cells: cells.__setitem__(3, "NO")))
    result = run(repo_copy)
    assert result.returncode != 0
...
def test_unclassified_fails_pass_package(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[4], lambda text: replace_row(text, 7, lambda cells: True, lambda cells: cells.__setitem__(2, "CLRMAPP_UNCLASSIFIED_FAIL")))
    result = run(repo_copy)
    assert result.returncode != 0
    assert "DECISION_UNCLASSIFIED_IN_PASS_PACKAGE" in result.stdout


def test_parser_ledger_inventory_row_removed_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[3], lambda text: remove_row(text, 9, lambda cells: cells[0] == "V1LOGIC-380"))
    result = run(repo_copy)
    assert result.returncode != 0
    assert "PARSER_LEDGER_INVENTORY_MISSING" in result.stdout


def test_concrete_output_without_source_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[3], lambda text: replace_row(text, 7, lambda cells: cells[1].startswith("RAW04-GOLDEN-"), lambda cells: cells.__setitem__(3, "balance=123")))
    result = run(repo_copy)
    assert result.returncode != 0
    assert "GOLDEN_EXPECTED_OUTPUT_SOURCE_INVALID" in result.stdout


@pytest.mark.parametrize("phrase", ["assumed", "probably", "standard parser", "typical clearinghouse", "expected structure"])
def test_unsupported_parser_language_fails(repo_copy: Path, phrase: str) -> None:
    ...
    result = run(repo_copy)
    assert result.returncode != 0
    assert "UNSUPPORTED_PARSER_LANGUAGE" in result.stdout
...
def test_fail_marker_fails(repo_copy: Path) -> None:
    mutate(repo_copy, FILES[3], lambda text: text.replace("RAW_REM_04_CLEARINGHOUSE_PARSER_BALANCE_LEDGER_MAPPING_PASS", "RAW_REM_04_CLEARINGHOUSE_PARSER_BALANCE_LEDGER_MAPPING_FAIL"))
    result = run(repo_copy)
    assert result.returncode != 0
